...
tota.wresults <- merge(tota, dat.predict[c("ForID", "Pred_fit.dens.forid_tot_pois")], by="ForID", all.x=T) 
``` 
...
tota.wresults$EstDensity <- tota.wresults$Pred_fit.dens.forid_tot_pois 
```
...
testset <- testset[order(testset$Pred_fit.dens.forid_tot_pois, decreasing = F),]
testset$PredxExposure <- testset$Pred_fit.dens.forid_tot_pois * testset$Exposure
testset$PredByExposure <- testset$Pred_fit.dens.forid_tot_pois / testset$Exposure
...
``` {r}
hist(tota.wresults$Pred_fit.dens.forid_tot_pois)
summary(tota.wresults$Pred_fit.dens.forid_tot_pois)
...
* To be careful, I'll examine the stand type with the highest count: °r tota.wresults[which.max(tota.wresults$Count),]$ForID° and the stand type with the highest predicted density `r tota.wresults[which.max(tota.wresults$Pred_fit.dens.forid_tot_pois),]$ForID"
...
kable(tmp[c("ForID", "Count", "EstDensity", "CalcObsDens", "Exposure", "Offset", "PKEY")], row.names=F)
mean(tmp$CalcObsDens)

tmp2 <- tota.wresults[tota.wresults$ForID %in% tota.wresults[which.max(tota.wresults$Pred_fit.dens.forid_tot_pois),]$ForID,]
tmp2$CalcObsDens <- tmp2$Count / tmp2$Exposure
kable(tmp2[c("ForID", "Count", "EstDensity", "CalcObsDens", "Exposure", "Offset", "PKEY")], row.names=F)
...
ggplot(tota.wresults, aes(x=as.factor(Rank), y=EstDensity, group=as.factor(Rank), fill=as.factor(Rank))) + geom_boxplot() + xlab("Matrix Stand Rank") + ylab("Bird Density (# pairs/ha)") + myfacettheme2 + guides(fill=FALSE) + scale_fill_manual(values=palrank) 
 
ggplot(tota.wresults, aes(x=as.factor(Rank), y=EstDensity, group=as.factor(Rank), col=as.factor(Rank), fill=as.factor(Rank))) + geom_violin() + xlab("Matrix Stand Rank") + ylab("Bird Density (# pairs/ha)") + myfacettheme2 + guides(fill=FALSE) +  guides(col=FALSE) + scale_fill_manual(values=palrank) + scale_colour_manual(values=palrank) 
``` 
...
ggplot(tota.wresults, aes(x=BecZone, y=EstDensity, col=EstDensity)) + geom_jitter(alpha=0.5,  position=position_jitter(height=0.1, width=0.2)) + myfacettheme3 +  ylab("Bird Density (# pairs/ha)") + scale_colour_gradientn(colours = paldens(10))
```
