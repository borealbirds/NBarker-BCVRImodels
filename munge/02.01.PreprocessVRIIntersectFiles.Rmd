...
Stands where we're missing information I need for modelling

* No age; PROJ_AGE_1 == NA
...
``` {r}
nrow(intersect_both[is.na(intersect_both$PROJ_AGE_1),])
head(intersect_both[is.na(intersect_both$PROJ_AGE_1),])
```
...
* Some stands had non productive descriptors or codes
* Others appeared to be national parks

``` {r}
table(intersect_both$NON_PRODUCTIVE_DESCRIPTOR_CD[is.na(intersect_both$PROJ_AGE_1)])
```



nrow(intersect_both[intersect_both$PROJ_AGE_1 %in% 0,])
Well actually age = 0 isn't missing information. It could indicate non-forested stands. e.g., non_productive_code = OR means the polygon is open range. This could still be useful for modelling bird densities...  
