...
colnames(vri)[which(colnames(vri) == "code_use")] <- "SpeciesGroup"
vri$ForID <- with(vri, paste(BecZone, SpeciesGroup, Age, Height, sep="."))
vri.qs <- vri[c("ForID", "Rank", "BecZone", "SpeciesGroup", "Age", "Height")] #quick save. 
...
``` {r, echo=T}
vri.0 <- vri.qs[vri.qs$Rank == "0",] #data frame where ranks are 0
...
forarea <- read.csv("data/ForestAreaCalc-long-2017.12.csv", header=T)
...
I cross-referenced some of these combinations of BecZone and Species Group with the original matrix (Migratory_bird_ranking_matrix_V1.3_Apr15_2016 FINAL-dataformat-zSpGroup.csv)
...
* Some forest stands exist with large areas but are not ranked in Kari's file. 
...
Even though some of these stands _should_ be ranked, they are not ranked in the Forsite shapefile. For the purposes of future comparisons, let's relable all 0 rank stands as "unranked"
...
# 2. The "FINAL" matrix file: "Migratory_bird_ranking_matrix_V1.3_Apr15_2016 FINAL.xlsx"
...
`r nrow(ranks[is.na(ranks$FinalRank),])` ForID are left to fill in. However, I still want to remove ranks for thsoe  stands with no forested area.
...
``` {r}
write.table(ranks, file="data/Ranks_final_long.csv", sep=",", row.names=F, col.names = T)
