...
birds <- birddata_preprocess2_withOffset
colnames(birds)
...
``` {r}
colnames(birds)[which(colnames(birds) %in% c("CORRECTION", "OFFSET"))] <- c("Exposure", "Offset")

bird.offsets <- birds[c("PKEY", "Exposure", "Offset")]
length(unique(birds$PKEY))
nrow(bird.offsets)

bird.offsets <- bird.offsets[!duplicated(bird.offsets$PKEY),]

vri_bird_tempaligned <- merge(vri_bird_tempaligned, bird.offsets, by="PKEY", all.x=T)
```
