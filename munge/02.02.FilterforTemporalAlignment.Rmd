...
#### Replace canfor's ReferenceYear with the YYYY from the bird surveys, since I agreed to accept these date. 
``` {r}
...
ggplot(vri_bird_nodist, aes(x=PROJ_AGE_1)) + geom_histogram(bins=40) + myfacettheme3 + ylab("Number of bird surveys") + xlab("Stand Age")
```

Take a look at the stands with young forests

``` {r}
vri_bird_nodist[vri_bird_nodist$PROJ_AGE_1 < 20,]
...
``` {r}
vri_birdsurveys2$DisturbanceYear <- as.integer(vri_birdsurveys2$DisturbanceYear)

vri_birdsurveys2[vri_birdsurveys2$DisturbanceYear < vri_birdsurveys2$YYYY,] 
                 
                 & vri_birdsurveys2$DisturbanceYear < vri_birdsurveys2$ReferenceYear,]
...
##### 2.a. A disturbance occurred after the inventory and before the bird survey. 




##### 2.b. A disturbance occurred after the bird survey and before the inventory. 
