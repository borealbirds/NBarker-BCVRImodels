...
* remove sites without dates. The following PCODEs all have at least one SS without a MM: `r unique(xy.pkey[is.na(xy.pkey$MM),]$PCODE)`
...
Testing to see which PKEY have no bird data, and which bird data have no associated spatial data

``` {r}
xy.pkey.pc1 <- merge(xy.pkey[c("PCODE", "SITE", "STN", "METHOD", "YYYY", "MM", "DD", "ROUND", "StartTime", "SS", "PKEY")], pcdat, by="PKEY", all.x =T) # merge with all pkey with xy coordinates, regardless of presence of bird data

head(xy.pkey.pc1[is.na(xy.pkey.pc1$ABUND),])
```
...
``` {r}
xy.pkey.pc2 <- merge(xy.pkey[c("PCODE", "SITE", "STN", "METHOD", "YYYY", "MM", "DD", "ROUND", "StartTime", "SS", "PKEY")], pcdat, by="PKEY", all.y =T) # merge with all abund data, regardless of presence of xy coordinates

head(xy.pkey.pc2[is.na(xy.pkey.pc2$PCODE),])

xy.pkey.pc2$PCODE_derived <- unlist(lapply(strsplit(as.character(xy.pkey.pc2$PKEY), ":"), function(x) {x[1]}))
...
xy.pkey.pc3 <- merge(xy.pkey[c("PCODE", "SITE", "STN", "METHOD", "YYYY", "MM", "DD", "ROUND", "StartTime", "SS", "PKEY")], pcdat, by="PKEY", all =T) # keeps all pkeys even if missing some data

```

``` {r}
xy.pkey.pc4 <- merge(xy.pkey[c("PCODE", "SITE", "STN", "METHOD", "YYYY", "MM", "DD", "ROUND", "StartTime", "SS", "PKEY")], pcdat, by="PKEY") # merge only where we have all data
...
##### Write the table that includes NAs, for now. 
``` {r}
write.table(xy.pkey.pc3, file="data/birddata_preprocess1.csv", sep=",", row.names=F, col.names=T)
```
...
2. **DD_duration_codes_methodology.csv**
3. **DD_distance_codes_methodology.csv**
...
projs <- projs[projs$PCODE %in% pkey$PCODE,] #projects I care about for now
projs <- projs[order(projs$PCODE),] #sort by pcode
projs$Maxdist <- toupper(projs$Maxdist)

kable(projs[c("PCODE", "METHOD", "DURMETH", "DISTMETH", "ChangeMethod", "MaxDuration", "Maxdist")], row.names=F)
```

Next, use table that merges xy.pkey with point count data (the one where eall data are intact, not missing)

``` {r merged.xy.pkey.pc, eval=T}
xy.pkey.pc <- xy.pkey.pc4
```

Next, merge pkey/point count data with project summary to associate each point count survey with a method. 

``` {r merge.xy.pkey.pc.method, eval=T}
xy.pkey.pc.meth <- merge(xy.pkey.pc, projs[c("PCODE", "METHOD", "DURMETH", "DISTMETH", "ChangeMethod", "MaxDuration", "Maxdist")], by="METHOD")
```

Create unique ID for combination of duration and distance method

``` {r}
xy.pkey.pc.meth$DurDisKey <- paste(xy.pkey.pc.meth$DURMETH, xy.pkey.pc.meth$DISTMETH, sep=".")
```

#### Compare bird counts among different survey types

`r length(unique(xy.pkey.pc.meth$DurDisKey))` different combinations of methods used, across `r length(unique(xy.pkey.pc.meth$PCODE))` different projects.

Subset to round1 and behaviour = 6 only

``` {r}
xy.pkey.pc.meth.round1 <- xy.pkey.pc.meth[xy.pkey.pc.meth$ROUND == 1 & xy.pkey.pc.meth$BEH == 6,]
```

Aggregate to "total bird" count per survey instance

``` {r aggregate, eval=T}
pkey.sumabund <- aggregate(xy.pkey.pc.meth.round1$ABUND,  by=list(PKEY=xy.pkey.pc.meth.round1$PKEY), FUN=sum)
...
pkey.method <- xy.pkey.pc.meth.round1[c("PKEY", "DISTMETH", "DURMETH", "METHOD", "MaxDuration", "Maxdist", "DurDisKey")]
