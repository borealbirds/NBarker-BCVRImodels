...
#### Calculate offsets for the missing ones
...
x$A <- ifelse(is.finite(x$MaxDistance), 
    x$MaxDistance^2*pi, tau_tota^2*phi_tota)
x$q <- ifelse(is.finite(x$MaxDistance), 
    edr_fun(x$MaxDistance, tau_tota), 1)
summary(x$p)
summary(x$A)
summary(x$q)
