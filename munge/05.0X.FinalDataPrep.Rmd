...
new.pkeys <- merge(new.pcodes, birddat4, by="PCODE", all.x=T) #PKEYS in those PCODES
nrow(new.pkeys) #how many PKEYS do we need new offsets for

new.ss <- merge(new.pcodes, birddat4[c("METHOD", "SS", "YYYY", "MaxDuration", "Maxdist", "PCODE")], by="PCODE", all.x=T) #PKEYS in those PCODES
...
``` {r}
tmp <- as.data.frame(do.call(rbind,lapply(offcovar, function(x) { length(unique(x))})))
colnames(tmp) <- c("Num.Unique.Values")
kable(tmp)

(has.offset.covars <- data.frame(PCODE=unique(offcovar$PCODE), hascovar="yes"))
new.pkeys <- merge(new.pkeys, has.offset.covars, by="PCODE", all.x=T)

new.pkeys$hascovar <- as.character(new.pkeys$hascovar)
new.pkeys$hascovar[is.na(new.pkeys$hascovar)] <- "no"
kable(rbind(head(new.pkeys), tail(new.pkeys)))

sum(new.pkeys$hascovar == "yes")
sum(new.pkeys$hascovar == "no")
nrow(new.pkeys)


(has.offsettz <- data.frame(PCODE=unique(offtz$PCODE), hasofftz="yes"))
new.pkeys <- merge(new.pkeys, has.offsettz, by="PCODE", all.x=T)

new.pkeys$hasofftz <- as.character(new.pkeys$hasofftz)
new.pkeys$hasofftz[is.na(new.pkeys$hasofftz)] <- "no"
kable(rbind(head(new.pkeys), tail(new.pkeys)))

sum(new.pkeys$hasofftz == "yes")
sum(new.pkeys$hasofftz == "no")
nrow(new.pkeys)
```

**Create table with PKEYs and other covariates needed to generate offsets**

``` {r}
covar.ss <- offtz[c("PCODE", "SS", "tree", "NALCMS05", "TZID", "FID_tz_wor")]
covar.ss <- covar.ss[!duplicated(covar.ss),]

new.pkeys.withcovar <- merge(new.pkeys[c("SS", "METHOD", "YYYY", "MaxDuration", "Maxdist", "hastotaoffset", "hascovar", "hasofftz")], covar.ss, by="SS", all.x = T)
