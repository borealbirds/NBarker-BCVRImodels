...
In previous steps/scripts, I cut out some columns and standardized for species groups. I also quality-checked and recalculated area. 
...
forarea <- read.csv("data/ForIDAreaCalc-long-2017.12.csv", header=T)
...
Load and do some quick fixes (SpeciesGroup column name, creation of ForID)
...
vri_forsite_zSpGroup$ForID <- with(vri_forsite_zSpGroup, paste(BecZone, SpeciesGroup, Age, Height, sep="."))

```

Merge corrected ranks with shapefile
``` {r}
vri_forsite_zSpGroup_Rankfixed <- merge(vri_forsite_zSpGroup, ranks, by="ForID", all.x=T)
cache("vri_forsite_zSpGroup_Rankfixed")
kable(vri_forsite_zSpGroup_Rankfixed[1:2, c(1:4, (ncol(vri_forsite_zSpGroup_Rankfixed) - 4):ncol(vri_forsite_zSpGroup_Rankfixed))], row.names=F)
```
