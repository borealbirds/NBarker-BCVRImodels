...
#### Adding the zeroes for other 

``` {r, eval=F}
pcdat_melt <- melt(pcdat[c("PKEY", "SPECIES", "DURATION", "DISTANCE", "ABUND", "BEH")], measure.vars="ABUND") # bring dataset into molten

#pcdat_wide <- cast(pcdat_melt, PKEY + DURATION + DISTANCE + BEH ~ SPECIES, value="ABUND") # reshape the dataset so we have ABUND as a function of PKEY, DURATION, DISTANCE, and  BEHAVIOUR 
```

Casting this shouldn't require aggregation. If it does, we'll need to explore.
...
* All of these duplicates involve the same Species code (UNGU) -- the duplicates are from when I merged in the English names!!
