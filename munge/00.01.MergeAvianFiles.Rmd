...
##### First, check for potential systematic differences among surveys
...
Peter suggests scaling all point counts to the same time/distance and then running analyses without a correction factor offset. This will be challenging if I want to include BBS data, which are unlimited distance surveys. 
...
kable(projs[c("PCODE", "METHOD", "DURMETH", "DISTMETH", "ChangeMethod", "MaxDuration", "Maxdist", "Bands.")], row.names=F)
```

Next, merge pkey table with point count data. 

``` {r merge.xy.pkey.pc, eval=T}
xy.pkey.pc <- merge(xy.pkey[c("SS", "METHOD", "ROUND", "PKEY", "YYYY", "MM", "DD", "StartTime", "obs")], pcdat, by="PKEY")
```
...
xy.pkey.pc.meth <- merge(xy.pkey.pc, projs[c("PCODE", "METHOD", "DURMETH", "DISTMETH", "ChangeMethod", "MaxDuration", "Maxdist", "Bands.")], by="METHOD")
```

Create unique ID for combination of duration and distance method

``` {r}
xy.pkey.pc.meth$DurDisCombo <- paste(xy.pkey.pc.meth$DURMETH, xy.pkey.pc.meth$DISTMETH, sep=".")

```

**Compare bird counts among different survey types**

`r length(unique(xy.pkey.pc.meth$DurDisCombo))` different combinations of methods used, across `r length(unique(xy.pkey.pc.meth$PCODE))` different projects.
...
``` {r aggregate, eval=T}
xy.pkey.pc.meth_tmp <- xy.pkey.pc.meth.round1[1:100,]
...
pkey.method <- xy.pkey.pc.meth.round1[c("PKEY", "DISTMETH", "DURMETH", "METHOD")]
pkey.method <- pkey.method[!duplicated(pkey.method),]
pkey.method$DistDurKey <- paste(pkey.method$DISTMETH, pkey.method$DURMETH, sep=".")
...
group.means <- aggregate(pkey.sumabund$sumABUND, by=list(DistDurKey=pkey.sumabund$DistDurKey), FUN=mean)
...
method.lookup <- pkey.method[c("DISTMETH", "DURMETH", "DistDurKey")]
...
group.means <- merge(group.means, method.lookup, by="DistDurKey")
group.means <- group.means[order(group.means$x, decreasing = T), c("DURATIONRANGE", "DISTANCERANGE", "x", "DURMETH", "DISTMETH")]

kable(group.means, row.names=F)
```
