...
### 1. Load export of VRI file

This is an export of the attribute table of the forsite "mig birds layer". Before exporting, I used "Calculate geometry" in ArcGIS to calculate the area, in hectares, of each polygon. After exporting, I standardized the tree species groups in the "TreeSpeciesCodes_Fix" script.

```{r load.vri.export.data, echo=T}
vri <- read.csv("data/mig_birds_Layer_polyarea_forstandatts-zSpGroup.csv")  
...
``` {r}
vri.mm <- merge(mastermatrix, vri.reduced[c("ForID", "OBJECTID", "polygon_area_cal")], by="ForID", all=T)  # ensure all forest stand types have at least one row associated with them, even if there's no forest in that stand type, and even if it's not one of the 3360 forest types we care about. 
```

##### Examine duplicates

``` {r, echo=T}
vri.dup <- vri.red[duplicated(vri.red$OBJECTID),] ## All 
...
``` {r echo=T}
myforarea <- aggregate(vri.mm$polygon_area_cal, list(ForID=vri.mm$ForID), FUN=function(x){sum(x, na.rm=T)}) # sum over polygons (objectids) with same forest stand id. 
kable(rbind(head(myforarea), tail(myforarea)), row.names=F)
```

**NOTES**
* `r length(unique(myforarea$ForID))` unique ForIDs in this aggregated dataset
* only `r length(unique(mastermatrix$ForID))` ForID combinations based on the set of attributes in documentation about the matrix
...
df.tmp <- myforarea[c("BEC", "SpeciesGroup", "ForAge", "ForHt", "area_Ha")]
...
mymat <- cast(df.tmp, BEC + SpeciesGroup ~ ForAge + ForHt ) 
...
df.tmp <- barb.mm[c("BEC", "SpeciesGroup", "ForAge", "ForHt", "area_Ha")]

colnames(df.tmp)[ncol(df.tmp)] <- "value"
barbmat <- cast(df.tmp, BEC + SpeciesGroup ~ ForAge + ForHt ) # cast into matrix format
```
...
* There are `r unique(comparea$DiffMoreThan.001)` stands where the difference in our calculations exceeds 0.001
...
write.table(myforarea[c("ForID", "MyArea")], "data/ForestAreaCalc-long-2017.12.csv", sep=",", col.names=T, row.names=F)
write.table(mymat, "data/ForestAreaCalc-wide-2017.12.csv", row.names=F, col.names=T, sep=",")  
```

``` {r, write.files.from.barbcalcs, echo=T}
write.table(barb.mm[c("ForID", "BarbArea")], "data/ForestAreaCalcBARB-long-2017.12.csv", sep=",", col.names=T, row.names=F)

write.table(barbmat, "data/ForestAreaCalcBARB-wide-2017.12.csv", row.names=F, col.names=T, sep=",")  
